...
VS_OUT VS_DirLightShader(VS_IN _in)
{
    ...
    // 사용하는 메쉬가 RectMesh( 로컬 스페이스에서 반지름 0.5짜리 정사각형)
    // 따라서 2배로 키워서 화면 전체가 픽쉘셰이더가 호출될 수 있게 한다.
...
PS_OUT PS_DirLightShader(VS_OUT _in)
{
    ...
    // 픽셀 위치랑 동일한 UV위치에서 Position 값을 가져왔는데, 해당 지점에 기록된 물체가 없다.
    ...
    // 광원 본인의 정보가 LightBuffer내에서 어디에 있는지 인덱스를 입력
    // 빛을 넣어줄 물체의 위치값과 노말값을 타겟 첵스쳐에서 가져와서 입력
